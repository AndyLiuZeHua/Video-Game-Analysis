...
```
## Problem
As a video game company, we dicide to launch a new game in 2017. In order to develop a successful game, we need to figure out three questions. What genre of the game should we create? What Platform should we use? Where should we release the game?
...
```{r}
data <- read.csv("D:\\columbia university\\AA\\R programming\\toolbox\\vgsales\\vgsales.csv", stringsAsFactors = F)
dim(data)
```
...
```{r}
data %>%
  filter(is.na(Name)|is.na(Platform)|is.na(Year)|is.na(Genre)|is.na(Publisher)|is.na(NA_Sales)|is.na(EU_Sales)|is.na(JP_Sales)|is.na(Other_Sales)|is.na(Global_Sales))
```
There are only two rows have missing value, deleting them will have no effect on the analysis.
```{r}
data <-
  data %>%
    filter(!(is.na(Name)|is.na(Platform)|is.na(Year)|is.na(Genre)|is.na(Publisher)|is.na(NA_Sales)|is.na(EU_Sales)|is.na(JP_Sales)|is.na(Other_Sales)|is.na(Global_Sales)))
```
Also notice that there are also missing value written as "N/a" in the dataset
```{r}
data %>%
...
data %>%
  ...
  ggvis(x = ~Year, y = input_select(c(SumGlo = "SumGlobalSales", sumNA = "SumNASales", sumEU = "SumEUSales", SumJP = "SumJPSales", SumOther = "SumOtherSales"), map = as.name)) %>% 
  layer_lines()
```
...
data %>%
  ...
  ggplot(aes(x=Year, y=sales, col=area)) +
  geom_point() +
